...
class BGCBSHelper
{
	...
	static function getPostOption($prefix=null)
	{
		...
		{
			if(preg_match('/^'.PLUGIN_BGCBS_OPTION_PREFIX.$prefix.'/',$key,$result)) 
			{
				$index=preg_replace('/^'.PLUGIN_BGCBS_OPTION_PREFIX.'_/','',$key);
				$option[$index]=$value;
			}
		}	
		
		$option=BGCBSHelper::stripslashes($option);
	...
	static function getGetOption($prefix=null,$useOptionPrefix=true)
	{
		...
		foreach($data as $key=>$value)
		{
			if((!preg_match('/^'.PLUGIN_BGCBS_OPTION_PREFIX.$prefix.'/',$key,$result)) && ($useOptionPrefix)) continue;
	...
	{
		if(!$display) return(PLUGIN_BGCBS_OPTION_PREFIX.'_'.$name);
		echo PLUGIN_BGCBS_OPTION_PREFIX.'_'.$name;
	}
	...
	{
		if($prefix) $name=PLUGIN_BGCBS_CONTEXT.'_'.$name;
	...
	static function displayAddress($data,$separator='<br>')
	{
		...
		if((int)BGCBSOption::getOption('address_format')===1)
		{
		...
		elseif((int)BGCBSOption::getOption('address_format')===2)
		{
			if($Validation->isNotEmpty($data['street']))
			{
				if($Validation->isNotEmpty($html)) $html.=$separator;
				$html.=trim($data['street_number'].' '.$data['street']);
			}			
		}
		...
		elseif((int)BGCBSOption::getOption('address_format')===2)
		{
			if($Validation->isNotEmpty($data['city']) && $Validation->isNotEmpty($data['postcode']))
			{
				if($Validation->isNotEmpty($html)) $html.=$separator;
				$html.=trim($data['city'].' '.$data['postcode']);
			}		
		}
		...
		'
			<li>
				...
				<div class="to-field-disabled">
					'.esc_html($post->ID).'
					<a href="#" class="to-copy-to-clipboard to-float-right" data-clipboard-text="'.esc_attr($post->ID).'" data-label-on-success="'.esc_attr__('Copied!','bookingo').'">'.esc_html__('Copy','bookingo').'</a>
				</div>
			</li>		
		';
	...
	static function displayDictionary($dictionary,$id)
	{
		$html=null;
		...
		if(!in_array(-1,$id))
		{
			foreach($id as $value)
			{
				if(array_key_exists($value,$dictionary)) 
				{
					...
					$html.='<a href="'.get_edit_post_link($value).'">'.esc_html($dictionary[$value]['post']->post_title).'</a>';
				}
			}
		}
		else $html='-';		
	...
	static function createAddPostLink($postType,$label=null)
	{
		if(is_null($label))
		{
			return(admin_url('post-new.php?post_type='.$postType));
		}
		else
		{
			return('<a href="'.esc_url(self::createAddPostLink($postType)).'" target="_blank" class="to-add-post">[+] '.esc_html($label).'</a>');
		}
	}
	
	/**************************************************************************/
	
	static function createEditPostLink($label,$id=0)
	{
		if($id!=0)
			return('<a href="'.esc_url(get_edit_post_link($id)).'" target="_blank" class="to-edit-post">[!] '.esc_html($label).'</a>');
		
		return('<a href="#" data-href="'.admin_url('post.php?action=edit&post=').'" target="_blank" class="to-edit-post">[!] '.esc_html($label).'</a>');
	}
    
    /**************************************************************************/
	
    static function filterPostTitle($postTitle,$postId)
    {
        $Validation=new BGCBSValidation();
        
        if($Validation->isNotEmpty($postTitle)) return($postTitle);
        
        return(sprintf(__('Post #%s','bookingo'),$postId));
    }
